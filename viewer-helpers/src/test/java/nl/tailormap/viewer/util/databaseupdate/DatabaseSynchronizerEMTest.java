...
import nl.tailormap.viewer.config.app.Application.TreeCache;
...
public class DatabaseSynchronizerEMTest extends DatabaseSynchronizerTestInterface {
    ...
    public void convertTestStartLevels() throws URISyntaxException, IOException, SQLException {
        ...
        Assertions.assertEquals(6, sls.size());
    }
    ...
    public void convertTestStartLayers() {
        ...
        Assertions.assertEquals(5, startLayers.size());
    }
    ...
    public void convertTestStartLevel() {
        ...
        Assertions.assertNotNull(sl, "StartLevel not found: conversion not correct");
        Assertions.assertEquals(Integer.valueOf(1), sl.getSelectedIndex());
        Assertions.assertEquals(level, sl.getLevel());
        Assertions.assertEquals(app, sl.getApplication());
    }
    ...
    public void applicationDeepCopyLevelTest() throws Exception {
        ...
        Assertions.assertEquals(oldLevels.size(), levelsCopy.size());

        for (Level level : levelsCopy) {
            Assertions.assertEquals(1, level.getStartLevels().size());
            for (StartLevel startLevel : level.getStartLevels().values()) {
                Assertions.assertEquals(copy, startLevel.getApplication());
            }
        }
    }
    ...
    public void applicationDeepCopyAppLayerTest() throws Exception {
        ...
        Assertions.assertEquals(oldAppLayers.size(), appLayers.size());

        for (ApplicationLayer appLayer : appLayers) {
            Assertions.assertEquals(1, appLayer.getStartLayers().size());
        }
    }
    ...
    public void testSelectedContentGeneration() throws SQLException, IOException, URISyntaxException, JSONException {
        ...
        assert (true);
        Assertions.assertEquals(6, app.getTreeCache().getLevels().size());
        Assertions.assertEquals(5, app.getTreeCache().getApplicationLayers().size());
        ...
        Assertions.assertEquals(expected.getString("rootLevel"), actual.getString("rootLevel"));
        ...
        Assertions.assertEquals(6, entityManager.createQuery("FROM Level").getResultList().size());
    }
    ...
    public void testUpdateApplicationLayerAttributesOrder() {
        ...
        for (ApplicationLayer appLayer : appLayers) {
            ...
            if (!areInCorrectOrder(attrs)) {
                Assertions.fail("Attributes of " + appLayer.getLayerName() + " not in correct order (should be alphabetically");
            }
        }
    ...
    public void testUpdateAttributeOrder() {
        ...
        if (!areInCorrectOrder(attrs)) {
            Assertions.fail("Attributes not in correct order (should be alphabetically");
        }
    }
