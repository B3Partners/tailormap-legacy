...
Ext.define("viewer.AppLoader", {
    ...
    loadApplication: function() {
        ...
        Ext.Ajax.request({
            ...
            failure: function(result) {
                if(failureFunction !== undefined) {
                    failureFunction(___(___("Ajax request failed with status ")) + result.status + " " + result.statusText + ": " + result.responseText);
                }
            }
        });
    },
    ...
    exposeVariable: function(variable, name, hideLog) {
        var warningMessage = ___("Warning! You are using global variable [var], please use FlamingoAppLoader.get('[var]').");
    ...
    updateLoginInfo: function() {
        ...
        if(link) {
            link.innerHTML = ___("Uitloggen");
            link.onclick = this.logout.bind(this);
        }
        var info = document.getElementById("loginInfo");
        if(info) {
            info.innerHTML = ___("Ingelogd als ") + "<b>" + this.config.user.name + "</b>";
        }
    },
