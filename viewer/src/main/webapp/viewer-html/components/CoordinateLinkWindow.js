...
Ext.define("viewer.components.CoordinateLinkWindow", {
    ...
    mapClicked: function (tool, event) {
        ...
        this.window = window.open(newUrl, 'name', 'height=' + this.config.height + ',width=' + this.config.width + ',location=no,resizable=yes,status=no,toolbar=no,menubar=no,resizable=yes');
        if (window.focus) {
            this.window.focus();
        }
    },
