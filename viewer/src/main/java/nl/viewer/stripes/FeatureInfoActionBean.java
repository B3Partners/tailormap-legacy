...
import nl.viewer.util.FeatureToJson;
        ...
        for(int i = 0; i < queries.length(); i++) {
            ...
            try {
                ...
                do {
                    ...
                    Filter currentFilter = filter != null && filter.trim().length() > 0 ? TailormapCQL.toFilter(filter,em) : null;
            ...
            for (int i = 0; i < response.getJSONArray("features").length(); i++) {
                ...
                for (FeatureTypeRelation rel : layer.getFeatureType().getRelations()) {
                    if (rel.getType().equals(FeatureTypeRelation.RELATE)) {
                        ...
                        Query q = new Query(fType.getTypeName(), TailormapCQL.toFilter(rightSide + "=" + jFeat.get(leftSide), em));
