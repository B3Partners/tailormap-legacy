...
public class TailormapCQL {
    ...
    private static Subselect createSubselect(String filter, EntityManager em) throws CQLException {
        ...
        Filter relatedFilter = TailormapCQL.toFilter(retrieveRelatedFilter(filter), em, false);
    ...
    private static List<Object> getFIDSFromRelatedFeatures(SimpleFeatureType sft, String filter, String column, EntityManager em) {
        ...
        try {
            FeatureSource fs = FeatureSourceFactoryHelper.openGeoToolsFeatureSource(sft);
            ...
            LOG.error("retrieving fids for RELATED_LAYER filter in TailormapCQL failed: " + ex);
        }
    ...
    private static String getUnionedFeatures(String filter, Long appLayerId, EntityManager em) throws CQLException {
        try {
            ...
            FeatureSource fs = FeatureSourceFactoryHelper.openGeoToolsFeatureSource(l.getFeatureType());
            ...
            LOG.error("retrieving geometry for intersects filter in TailormapCQL failed: " + ex);
        }
        return null;
    }
}
