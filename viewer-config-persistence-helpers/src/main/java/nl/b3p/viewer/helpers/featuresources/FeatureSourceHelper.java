...
public class FeatureSourceHelper {
    private static final Log log = LogFactory.getLog(FeatureSourceHelper.class);
    ...
    public static FeatureSourceUpdateResult update(EntityManager em, FeatureSource fs) throws Exception{
        ...
        try{
            List<SimpleFeatureType> newFeatureTypes = SourceFactoryHelper.createFeatureTypes(fs,result.getWaitPageStatus().subtask("",80));
            int processed = 0;
    ...
    public static List<String> calculateUniqueValues(SimpleFeatureType sft, String attributeName, Filter filter) throws Exception{
        return FeatureSourceHelper.calculateUniqueValues(sft, attributeName, MAX_FEATURES_DEFAULT, filter);
    }
    ...
    public static Map<String, String> getKeyValuePairs(SimpleFeatureType sft, String key, String label, int maxFeatures) throws Exception {
        ...
        try {
            fs = (SimpleFeatureSource) SourceFactoryHelper.openGeoToolsFeatureSource(sft);
